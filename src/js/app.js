App = {
  ...
  render: function() {
      ...
      .then(async function(chainclubInstance) {
        ...
        for (var i = 0; i < App.polls.length; i++) {
          if (App.polls[i].type == "boolean") {
            $("#polls-list").append(
              `<li>${App.polls[i].subject} (${App.polls[i].num_votes} votes) <button index=${i} vote=true class=yes >Yes</button> <button index=${i} class=no vote=false >No</button></li>`
            );
    ...
    function vote(value, bool) {
      App.contracts.Chainclub.deployed().then(function(instance) {
        instance
          .voteOnBooleanPool(value, bool, { from: App.account })
